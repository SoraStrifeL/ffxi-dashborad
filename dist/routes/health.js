...
function createHealthRouter(pool) {
    ...
    router.get('/api/me/permissions', auth_1.requireAuth, (req, res) => {
        const tier = req.user.tier;
        res.json({
            login: req.user.login,
            tier,
            permissions: rbac_1.ROLE_PERMISSIONS[tier] ?? [],
        });
    });
    // ── OpenAPI 3.0 spec ───────────────────────────────────────────────────────
    router.get('/api/openapi.json', auth_1.requireAuth, (0, rbac_1.requirePermission)('view:accounts'), (_req, res) => {
