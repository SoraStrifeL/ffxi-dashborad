...
import { datEnabled, getStrings, stringResourceKeys } from '../dat';

export function createDatRouter(): Router {
  const router = Router();

  // Feature status + available string resources (public-ish: just enabled flag).
  router.get('/api/dat/status', requireAuth, (_req, res) => {
    res.json({ enabled: datEnabled(), resources: stringResourceKeys() });
  });
